...
| **Grok** | Same plugin package (`.grok-plugin/plugin.json`), same hooks convention | **Not wired on the machine this was developed on.** A protected command run inside a real Grok session writes no record. Grok installs and enables the plugin and its `inspect` lists the hooks, and earlier real Grok tool calls did reach the gate — so the wiring is not absent in principle — but it does not currently fire, and no claim is made that it does. The same probe caveat as Codex applies. `bin/` exposure unconfirmed. |
