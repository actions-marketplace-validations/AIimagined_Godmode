...
# code-of-law is not compiled at all (2026-08-29, two field rounds in one
# night): capping it to ADVISORY still minted twenty advisory rules the
# checkability review then demanded decisions for - per law, per edit,
# forever. The law file is DELIVERY, not source: its guards already reach
# every session through the brief's top laws and the wrapper skill, each
# with archive provenance; compiling them again as charter rules is the
# same guard delivered twice under two ids. The role stays bound in the
# corpus - attest and the required-sources counter still demand the file
# be read - the charter just never mints rules from it.
SKIPPED_ROLES: frozenset[str] = frozenset({"code-of-law"})
...
def compile_binding(binding: Binding, project: Path) -> list[Rule]:
    rules: list[Rule] = []
    seen: set[str] = set()
    if binding.role in SKIPPED_ROLES:
        return rules
