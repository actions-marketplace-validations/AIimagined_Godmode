...
- `DEGRADED` - a live proof was on record and PROVABLY fresh (session-
  ...
  record, an expired TTL, or a `hook_version`/`trusted_hook_hash` mismatch
  between what the proof recorded and what is true right now. "Previously
  HARD, now broken" - the regression case CX-1's binary scale could not
  distinguish from a fresh install.
...
hook script's own bytes, when a path was resolvable), `nonce_hash` (sha256
of `request_id`, redundant with the field itself but named per the CX-5
contract), `observed_decision` (what the hook actually rendered - `"deny"`
for every probe this codebase writes today), `host_acknowledgement` (a
best-effort bool from structural registration state, `None` when
unresolvable - never a live shell-out from inside a record write),
`expiry` (an ISO-8601 timestamp, `PROOF_TTL_SECONDS` after mint). Freshness
for `HARD` now means "session-anchored AND unexpired" - both, not either.

**Backward compatibility, stated honestly rather than silently patched
around:** a CX-1-shaped minimal record (no `expiry` key at all - every
proof this codebase wrote before this change) still reads without error,
and can never claim `HARD` again - a proof without an expiry cannot prove
"unexpired," so it grades at most `PARTIAL`. This is a real behavior change
for any archive holding pre-CX-5 proof records, disclosed here rather than
treated as a compatibility shim: the old shape proved less than it claimed
to, and CX-5 stops crediting it for more than it can actually show.
...
def record_interception_proof(
    ...
    *, hook_script: Path | None = None, observed_decision: str = "deny",
    host_acknowledgement: bool | None = None, ttl_seconds: int = PROOF_TTL_SECONDS,
) -> dict[str, Any]:
    ...
    `hook_script`, when given and readable, is hashed into
    `trusted_hook_hash` - the CX-5 drift signal `interception_state` compares
    against the CURRENT file's hash on every later read, so a hook silently
    edited after the proof was written (this project's own git-backstop
    tamper detector proves that class of edit is real, not hypothetical)
    downgrades a once-HARD proof to `DEGRADED` instead of staying HARD
    forever on stale evidence. Omitted (`None`) when the caller has no
    resolvable path (e.g. `godmode_githooks.run_git_verify`'s scratch-repo
    verify, which proves the git backstop, not this hook file) - the field
    is simply absent, never a guessed or blank hash that could coincidentally
    "match" a tampered file's absence of one.
    """
    data: dict[str, Any] = {
        "host": str(host)[:80],
        "tool": str(tool)[:80],
    ...
    trusted_hash = _hash_file(hook_script) if hook_script is not None else None
...
def interception_state(
    ...
    data = proof.get("data", {})
    enriched = isinstance(data.get("expiry"), str) and bool(data["expiry"])
    ...
    if not enriched:
        # CX-1-shaped minimal record: fresh by session anchor, but carries
        # none of CX-5's freshness/identity fields, so it can never itself
        # prove "unexpired" or "untampered." Capped at PARTIAL, exactly as
        # the migration note in the module docstring promises - never
        # silently upgraded to HARD on a shape that cannot support the
        # claim.
        return LEVEL_PARTIAL

    if _expiry_passed(data["expiry"], now) or _version_drifted(data) or _hash_drifted(data, hook_script):
        return LEVEL_DEGRADED
