...
| **Codex** | Same plugin package (`.codex-plugin/plugin.json`), same hooks convention | **Not wired on the machine this was developed on.** Codex's `/hooks` panel reports `Installed: 0` for every event, and a protected command run inside a real Codex session writes no record — so the pre-tool gate does not run there. The plugin itself installs and enables (`codex plugin list` shows it), so skills and the CLI work; the hook boundary does not. `godmode hooks probe --host codex` reaching `HARD` does NOT contradict this: the probe self-injects into the hook script and its own docstring states it "does NOT prove a live host's own runtime is wired to call this script on real tool calls". |
| **Grok** | Same plugin package (`.grok-plugin/plugin.json`), same hooks convention | **Live-proven (2026-08-28 field report, on Windows).** Real tool calls in a live Grok session run the gate: a protected command and an unmapped tool were denied and the host honored the deny (the tools did not run), Grok's own read-only builtins pass as read-kind, and `godmode hooks status` reads HARD from a fresh probe whose proof record carries the host's acknowledgement. Grok expands `${CLAUDE_PLUGIN_ROOT}` through its documented alias and parses the shared shell-form `hooks/hooks.json`. Grok has no `ask` decision, so a would-ask folds to deny with the staged-capability remedy. |
