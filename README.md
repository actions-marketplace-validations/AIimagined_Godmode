...
| **Codex** | Same plugin package (`.codex-plugin/plugin.json`), same hooks convention | Live: a probe run from inside a real Codex session records an interception proof and `hooks status --host codex` reads `HARD`. Codex fires the shared CamelCase `PreToolUse` block and sends `tool_name: "Bash"` with `tool_input.command`. Its hooks require review in Codex's own `/hooks` trust panel before they run. Whether Codex's tool sandbox adds `bin/` to a command's PATH the way Claude Code's does isn't established here. |
| **Grok** | Same plugin package (`.grok-plugin/plugin.json`), same hooks convention | Live: a probe run from inside a real Grok session records an interception proof and `hooks status --host grok` reads `HARD`. Grok loads the plugin through Claude compatibility and sends Claude's tool names. `bin/` exposure unconfirmed. |
