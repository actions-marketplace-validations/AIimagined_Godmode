...
- Codex's pre-tool boundary reached HARD for the first time, and settled a question that had blocked the release. Driven from inside a real Codex session, the payload-capture probe recorded `{"event": "PreToolUse", "tool": "Bash", "field_names": ["command"]}` - Codex fires from the shared manifest's CamelCase block and sends Claude's tool names, which refutes the long-standing leading theory that Codex was rejecting the hooks file over its double-cased keys: the file parses, the hook runs, and the adapter simply did not recognise the names it was being sent. Codex and Grok both resolve Claude-named tools now, through one shared helper reading the Claude adapter's own tool map rather than a third transcription of it - two copies of one list is what caused this in the first place. Separately, running three hosts' probes in a single session exposed that supersession was not host-scoped: any newer `probe-failed`/`hook-uninstalled`/`hook-health-degraded` record degraded every host's proof regardless of which host it belonged to, so a Codex failure reported that Claude's freshly proven boundary had "demonstrably broken". Supersession is per-host evidence now, with a record carrying no host at all still superseding for everyone, since an unattributed breakage cannot be ruled out for any of them. All three installed hosts verify HARD with recorded proofs and no degradation.
- Codex file edits reached no boundary at all, and the cause was two event names that Codex does not have. The shared hooks manifest declared `session_start` and `pre_tool_use` alongside the CamelCase events; OpenAI's published hooks reference lists every Codex event verbatim and all of them are CamelCase, with no snake_case spelling anywhere. Those two keys named events Codex cannot fire, which is what left the orphan trust row in the operator's Codex config carrying a hash and no enabled flag - and, far more seriously, left `apply_patch`, Codex's documented file-edit tool, matched only under a key that never fires. Proven live: a Codex session created a file through `apply_patch` and wrote zero archive records, where a shell call through the same session reaches a HARD interception proof. Unlike the other findings in this batch, which all failed closed, this one was open by omission. Codex now rides the shared CamelCase events it was already firing, resolving the documented legacy root alias, and its tool names join that matcher so file edits are gated like every other tool. Regeneration actively retires the two invalid keys rather than merely ceasing to emit them, so an existing checkout heals itself. The casing tripwire that recorded this as an open question is updated to record the answer: the duplicate-key parse-failure theory is refuted, because the file parsed and the hook ran throughout.
- Cursor carried the same manifest-versus-adapter split that left Grok unable to record an interception proof, and it was found statically - Cursor is not installed on the machine this was written on. `.cursor-plugin/hooks.json` subscribes `preToolUse` to `Shell|Write|Delete`, but Cursor was routed through the generic adapter, whose tool map knows `Write` and neither `Shell` nor `Delete`: two of the three tool types godmode's own manifest asks Cursor to send would have come back `unrecognized-tool`, so every shell command and every delete would have been refused as an unknown tool and no proof could have been written. Cursor now has its own adapter built from its documented dialect (Addendum 5, re-verified against Cursor's published hooks reference): `Shell` reads `tool_input.command` as documented, and `Write`/`Delete` read the first present of the documented path spellings, with a `Delete` whose target cannot be read still classified as a delete rather than as an unknown tool - naming what is known beats answering "unrecognized" for a tool whose name is documented. The drift guard that found it now walks every shipped manifest rather than Grok's alone, so a matcher that widens without its adapter widening with it fails a test instead of a host. `MultiEdit` joins the edit family for the same reason. Codex's `functions.exec` is exempt by construction and stays so: it is an orchestration wrapper, and failing closed on anything but a nested call is its documented behaviour.
- Gemini CLI's gate recognised none of Gemini's tools. The shipped hooks fragment subscribes `BeforeTool` with a `.*` matcher - every tool - while Gemini was routed through the generic adapter, whose map holds only Claude's tool names, so every Gemini tool call arrived as an unknown tool and was refused: `run_shell_command`, the tool Gemini runs shell commands with, included. Gemini now has an adapter built from its published hooks reference: `run_shell_command` carries its text in the command parameter, `write_file` and `replace` are fenced edits, `read_file` is a read, and an undocumented name still fails closed. Its blocking contract needed no change - Gemini requires `decision` plus a mandatory `reason`, and the dual-output envelope already carries both. Because a wildcard matcher cannot be enumerated the way a name list can, the guard for Gemini pins the documented tool names instead, and separately checks the fragment declares only events that appear in Gemini's published event list. The latency-budget lookup for Codex was still reading the retired snake_case key and therefore reporting its timeout budget as unknown; it now reads the event Codex actually fires. Also verified against the Agent Plugins Specification 1.1.0: the portable base manifest carries every one of its ten permitted top-level fields and nothing else, with the required schema identifier present, so it conforms to the closed schema - and since that specification requires clients to report and ignore unknown fields rather than reject the plugin, carrying a host-specific hooks key in a host-specific manifest directory cannot cost ingestion anywhere.
- Grok's pre-tool gate never recorded an interception proof, and the cause was two files in this repository disagreeing about one list. The spec bound the Grok matcher as a union - Claude's tool names plus Grok's own - and `.grok-plugin/hooks.json` honours it, subscribing PreToolUse to nine names (`Bash|PowerShell|Write|Edit|MultiEdit|NotebookEdit|run_terminal_command|search_replace|write`). The host-event adapter implemented only Grok's three, so six of the names godmode's own manifest asks Grok to send were answered with `unrecognized-tool`. Enforcement was never weakened - every such call failed closed and was denied - but a tool the adapter cannot name cannot have a proof written for it, so `godmode hooks probe --host grok` reported UNAVAILABLE indefinitely. The adapter now resolves the Claude-named half through the same tool map and tool-kind sets the Claude adapter reads, rather than a second transcription, and a new test reads the shipped manifest's own matcher and asserts every name in it resolves - the drift guard whose absence let the two separate. That guard immediately found a second instance: `MultiEdit` was subscribed by the matcher but named by no tool map at all, and now joins the edit family alongside Write/Edit/NotebookEdit under the same fence. Verified live: a probe run from inside a real Grok session reaches `state: HARD` with `proof_recorded: true`, where the same command previously returned UNAVAILABLE, and hook latency fell from 1342 ms to 160 ms once calls stopped falling through the unrecognized path.
- The shipped host claims said less than the evidence supports, and the coverage map's own reconcile rule made that a live inconsistency rather than merely stale prose. Codex and Grok were both described as packaged but never independently live-probed, and the interception row explicitly withheld a HARD grade for either "until an operator runs the live-host layer against that host's own real binary and it passes" - a condition that has now been met on this machine, from inside each host's own real session, with an interception proof recorded and `hooks status` reading HARD for both. Those two rows now state what was demonstrated: which event block each host fires, which tool names it sends, and for Codex that its hooks require review in its own trust panel before they run. A proof is per-machine and per-install, so the coverage map says so plainly rather than letting one operator's receipt read as a claim about anyone else's. Cursor and Gemini are unchanged in substance and stay unproven, because neither host is installed here - but their row now records that both adapters are built from those hosts' own published hook references rather than guessed, and that a checked-in test asserts every tool name each shipped manifest subscribes to resolves in the adapter that host uses.
- The host support table and coverage map said a probe reaches HARD without saying that the grade is deliberately per-session. A reader could reasonably have taken "reads HARD" to mean the host stays proven, when a later session anchor returns it to PARTIAL until the probe runs again - which is the entire point of anchoring proofs, since otherwise a months-old receipt would read as current enforcement forever. Both surfaces now say so plainly, including the detail that running this repository's own test suite is enough to age a proof out, because its dogfooding layer opens a real session here. Nothing about the mechanism changed; the claim now matches it.
